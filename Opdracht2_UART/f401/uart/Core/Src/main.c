...
int main(void)
{
  ...
  //DIt is geen optimaal gebruik van geheugen.
  uint8_t buffer[20] = "";
  const uint8_t on[20] = "on";
  const uint8_t off[20] = "off";
  const uint8_t nextLine[2] = "\r\n";
  uint32_t timeout = 50;
  ...
  while (1)
  {
	  ...
	  HAL_Delay(10);
	  //HAL_UART_Transmit(&huart2,Test,sizeof(Test),timeout);
